...
contract TChokeCamelotV2Handler is Context {
    ...
    function handleDeposit(
        ...
        address from,
        uint256 positionID
    ) external onlyTChoke returns (bytes32, uint256) {
        bytes32 depositID = _calculateDepositID(
            liquiditySource,
            from,
            positionID
        );
        ...
        if (
            ...
            TChoke.depositIDtoDepositor(depositID) != address(0) ||
    ...
    function handleWithdraw(
        ...
        address to,
        uint256 positionID
    ) external onlyTChoke returns (bytes32) {
        bytes32 depositID = _calculateDepositID(
            liquiditySource,
            to,
            positionID
        ...
        if (
            TChoke.deposits(depositID) == 0 ||
            tokenIdToDepositor[positionID] != to ||
    ...
    function receiptsBalance(address _owner) external view returns (uint256) {
        require(
            _owner != address(0),
            "TChoke: balance query for the zero address"
        );
        return _receipts[_owner].length();
    }

    function receiptOfOwnerByIndex(
        address _owner,
        uint256 _index
    ...
    function getDepositInformation(
        ...
        (
            ...
            uint256 lpAmount,
            uint256 totalSupply
        ) = _calculateUSDProvided(source, positionID);
    ...
    function getDepositInformation(
        ...
        return (
            ...
            lpAmount,
            totalSupply,
            TChoke.deposits(depositID)
        );
    }

    function _calculateUSDProvided(
        ILiquiditySource source,
        uint256 positionID
    ) private view returns (uint256, uint256, uint256) {
        (address lpTokenAddress, , , , , , , ) = source.getPoolInfo();
        (uint256 lpAmount, , , , , , , ) = source.getStakingPosition(
            positionID
        );

        ICamelotPair camelotPair = ICamelotPair(lpTokenAddress);
        ...
        usdProvided =
            ((usdcReserve * lpAmount) / totalSupply) *
            (10 ** (TChoke.decimals() - USDC_DECIMALS));
     ...
     *  - The address of this smart contract (representing each instance of TChokeCamelotV2Handler)
